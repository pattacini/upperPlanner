...
::getBestTrajectory (std::list<double*>& trajectoryOut) {
    ...
    while (vertexCurr) {
        ...
        stateArrCurr[2] = stateCurr[2]; //Nguyen Dong Hai Phuong: added to fix the problem with z cooridnate of origin version
        
        trajectoryOut.push_front (stateArrCurr);
        
        Vertex<State,Trajectory,System>& vertexParent = vertexCurr->getParent(); 
        
        // Nguyen Dong Hai Phuong: comment out to remove the copy of vertex in trajectory in the original version
        /*
        if (&vertexParent != NULL) {
            ...
            for (std::list<double*>::iterator iter = trajectory.begin(); iter != trajectory.end(); iter++) {
                ...
                stateArrCurr[2] = stateArrFromParentCurr[2];  //Nguyen Dong Hai Phuong: added to fix the problem with z cooridnate of origin version
